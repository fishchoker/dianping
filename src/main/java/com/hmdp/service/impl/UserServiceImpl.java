...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public Result sendCode(String phone, HttpSession session) {
        ...
        //符合，生成验证码
        String code = RandomUtil.randomNumbers(6);//import cn.hutool.core.util.RandomUtil;
        ...
        // 返回ok
        return Result.ok();//表示成功执行
    }
	@Override
	public Result login(LoginFormDTO loginForm, HttpSession session)
	{
		//校验手机号和验证码 每一个请求都做独立校验
		...
		/*
		 * Object session_code=session.getAttribute("code"); String
		 * code=loginForm.getCode(); //不一致
		 * if(session_code==null||!session_code.toString().equals(code)) { return
		 * Result.fail("验证码错误！"); }
		 */
		//从redis获取验证码
		Object redis_code=stringRedisTemplate.opsForValue().get(RedisConstants.LOGIN_CODE_KEY+phone);
		String code=loginForm.getCode();
		if(redis_code==null||!redis_code.equals(code)) 
		{ return Result.fail("验证码错误！"); }
		
		//一致 根据手机号查用户 select* from tb_user where phone =?
		...
		/*
		 * //保存用户信息 session.setAttribute("user", user);
		 */
		//保存到redis中 哈希：key为随机生成的token token需要返回到客户端
		
		//随机生成token 使用UUID
		 String token = UUID.randomUUID().toString(true);
		//将user对象转为哈希
		 UserDTO userDTO = new UserDTO();
		 BeanUtil.copyProperties(user, userDTO); // 修改拷贝顺序，确保从 user 拷贝到 userDTO
		//保存到redis
		 //需要把user转变成map
		 ...
		 //这里long类型的id没法转string 存不到redis里会报错 重新定义一个
		stringRedisTemplate.opsForHash().putAll(RedisConstants.LOGIN_USER_KEY+token, userMap);//存储userDTO的所有字段
		//由于redis是内存数据库，需要设置过期时间 30min
		stringRedisTemplate.expire(RedisConstants.LOGIN_USER_KEY+token,RedisConstants.LOGIN_USER_TTL,TimeUnit.MINUTES);
		//访问时需要更新数据的有效期
		//从登录拦截校验那里得到访问请求 修改拦截器
		//增加一个拦截所有的拦截器用来识别请求->然后刷新有效期
		//不需要返回登陆凭证 自动写入cookie
		//调用写1k个用户的方法
		//generateToken();
		return Result.ok(token);//记得改！！
	}
	...
	public void generateToken() {
        ...
        for(String phone : phoneNumbers){
            ...
            //7.1 随机生成Token作为登录令牌
   		 	String token = UUID.randomUUID().toString(true);
            String filePath = "C:\\Users\\ajbjs\\Desktop\\output.txt";
	...
	public Result signCount() {
      ...
      //5.获取本月截止今天为止的所有的签到记录，返回的是一个十进制的数字
        //BITFIELD key get u[dayofmonth]0 从 Redis 键 key 中存储的数据的第一个比特位开始，获取长度为 dayOfMonth 的比特位
